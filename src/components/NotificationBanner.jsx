...
const NotificationBanner = () => {
  ...
  const handleEnable = async () => {
    ...
    const subscribed = granted && (await hasActivePushSubscription());
    setVisible(!subscribed && Notification.permission !== "denied");
  };
  ...
  return (
    <div className="mx-3 md:mx-6 mt-2 p-3 rounded-xl bg-blue-600/20 border border-blue-500/40 flex items-center justify-between gap-2">
      <p className="text-xs md:text-sm text-slate-200">
        App band hone par bhi alerts — notifications enable karein
      </p>
      <button
        type="button"
